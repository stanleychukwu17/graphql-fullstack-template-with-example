...
func Setup_continuous_integration_db(which_db string) (string, testcontainers.Container, error) {
	...
	if which_db == "postgres" {
		...
		dsn := fmt.Sprintf(
			"%s:%s@tcp(%s:%s)/%s?charset=utf8mb4", db_user, db_password, host, port.Port(), db_name,
		)
...
func Connect_to_development_database(which_db string) (db *gorm.DB, err error) {
	...
	if which_db == "postgres" {
		...
		dsn = fmt.Sprintf("host=localhost user=%v password=%v dbname=%v port=%v sslmode=disable", DB_USER, DB_PASSWORD, DB_NAME, DB_PORT)
		db, err = gorm.Open(postgres.Open(dsn), &gorm.Config{
			Logger: logger.Default.LogMode(logger.Silent), // Only log errors
		})
	} else if which_db == "mysql" {
		...
		dsn = fmt.Sprintf("%s:%s@tcp(localhost:%s)/%s?charset=utf8mb4", DB_USER, DB_PASSWORD, DB_PORT, DB_NAME)
		db, err = gorm.Open(mysql.Open(dsn), &gorm.Config{
			Logger: logger.Default.LogMode(logger.Silent), // Only log errors
		})
	}
